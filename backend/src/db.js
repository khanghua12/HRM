...
async function query(sql, params = {}) {
  const p = getPool();
  if (!p) {
    throw new Error('USE_MYSQL=false');
  }
...
async function ping() {
  if (!isMySqlEnabled()) {
    return { enabled: false, ok: false, message: 'USE_MYSQL=false' };
  }
...
async function upsertEmployeeAccount(employeeId, payload) {
  ...
  try {
    ...
    for (const menuKey of payload.menuPermissions) {
      await conn.execute(
        `
          INSERT INTO account_menu_permissions (account_id, menu_key)
          VALUES (:accountId, :menuKey)
        `,
        { accountId, menuKey }
      );
    }
